...
**The macOS updater archive is rebuilt from the final signed .app (fixed 2026-09-07).** v0.3.19's shipped `Redline_0.3.19_aarch64.app.tar.gz` was generated by `tauri:build` before dylib bundling and before the final codesign, so it shipped with zero `Contents/Frameworks` entries and every binary linking Tesseract/Leptonica/libarchive by absolute Homebrew path - a Mac without Homebrew installed could not launch after auto-updating. `build-releases.yml` now has a "Regenerate updater archive (post-bundle, post-sign)" step that re-tars and re-signs the archive from the final `.app`, plus a "Verify updater archive integrity (hard gate)" step that fails the build if the archive is missing bundled OCR dylibs or if `otool -L` shows any `/opt/homebrew` path. See `docs/ocr.md` for the full account.
...
4. Download the macOS `.app.tar.gz` from the Forgejo release and extract it - confirm `Contents/Frameworks/` contains the OCR dylibs (`libtesseract`, `libleptonica`, etc.) and run `otool -L` on the main executable to confirm no `/opt/homebrew` paths remain. The CI gate (`Verify updater archive integrity`) already checks this in the build job, but a shipped-artifact spot-check catches anything the gate itself might miss.
5. Re-releasing under a fresh number (e.g. burn a bad tag, ship the next patch) is cleaner than force-rewriting a tag when an auto-updater is in play.
